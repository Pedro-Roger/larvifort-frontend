...
- [x] Módulo de Quadros e Colunas (`API-003`)
  - [x] `GET /tasks/boards` (listar quadros/setores)
  - [x] `POST /tasks/boards` (criar quadro)
  - [x] `PATCH /tasks/boards/:id` (atualizar quadro)
  - [x] `DELETE /tasks/boards/:id` (remover quadro)
  - [x] `GET /tasks/boards/:boardId/columns` (listar colunas do quadro)
  - [x] `POST /tasks/boards/:boardId/columns` (criar coluna com triggerAction)
  - [x] `PATCH /tasks/columns/:id` (atualizar coluna)
  - [x] `DELETE /tasks/columns/:id` (remover coluna migrando tarefas via `moveToColumnId`)
- [x] Módulo Operacional de Tarefas, Herança e Anexos (`API-004`)
  - [x] `GET /tasks` (filtros por `boardId`, `columnId`, `search`, `status`, `assigneeId`, `parentId`)
  - [x] `GET /tasks/:id` (detalhes completos com anexos, histórico, ordem de pedido e tarefas-filhas)
  - [x] `POST /tasks` (criação com suporte a `parentId`, `orderId`, `phone`)
  - [x] `PATCH /tasks/:id` (atualização completa)
  - [x] `PATCH /tasks/:id/status` (movimentação com trigger de automação)
  - [x] `POST /tasks/:id/transfer` (hand-off intersetorial: modo MOVE ou CHILD_TASK com nota)
  - [x] `DELETE /tasks/:id` (exclusão de tarefa)
  - [x] `POST /tasks/:id/attachments` (upload multipart de anexo)
  - [x] `DELETE /tasks/:id/attachments/:attachmentId` (excluir anexo)
- [x] Módulo de Vínculo de Ordens de Pedido (`API-005`)
  - [x] `GET /orders` (listagem com busca e filtro por cliente)
  - [x] `GET /orders/:id` (detalhes da ordem para visualização na tarefa)
