...
export type ReservationStatus = "ACTIVA" | "CANCELADA" | "CONSUMIDA";
export type StockReservation = { id: string; orderId: string; orderNumber: string | null; productName: string; quantity: number; unit: string; unitName: string; locationName: string; status: ReservationStatus; createdAt: string };
...
export function fetchReservations() { return apiGet<unknown>("/stock/reservations?status=ACTIVA").then(normalizeReservations); }
